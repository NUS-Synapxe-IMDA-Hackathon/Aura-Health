...
const VITALS_INFO: Record<string, string> = {
  'Avg Heart Rate':   'Average beats per minute recorded throughout the sleep session. Elevated rates may indicate stress or restlessness.',
  'Avg Respiration':  'Average breaths per minute during sleep. Normal range is 12–20 breaths/min.',
  'Apnea Events':     'Number of breathing pauses (≥10 sec) detected. Zero is normal; elevated counts may indicate sleep apnea.',
  'Turns in Bed':     'Total body movements detected. A high count may indicate restless or fragmented sleep.',
  'Sleep Onset':      'Minutes taken to fall asleep after going to bed. Under 20 minutes is considered typical.',
  'WASO':             'Wake After Sleep Onset — total minutes spent awake after initially falling asleep. Under 30 min is typical.',
  'Times Out of Bed': 'Number of times the resident left the bed during the night. Frequent trips may signal discomfort.',
  'Efficiency':       'Ratio of time asleep to total time in bed. 85% or above is considered good sleep efficiency.',
}
...
export function SleepPage() {
  ...
  if (!s && !loading) {
    return (
      <div className="px-5 pt-4">
        ...
        <p className="text-[13px] text-slate-400 mt-1 mb-1">{error ?? 'No sleep data available.'}</p>
        {error && <p className="text-[11px] text-slate-300 font-mono break-all">{error}</p>}
      </div>
    )
  }

  // ── Computed display values (-- when loading) ────────────────────────────
  const deepPct    = s ? Math.round((s.total_deep_minutes  / s.time_in_bed_minutes) * 100) : 0
  const lightPct   = s ? Math.round((s.total_light_minutes / s.time_in_bed_minutes) * 100) : 0
  const awakePct   = s ? Math.round((s.total_awake_minutes / s.time_in_bed_minutes) * 100) : 0
  const scorePercent = s?.sleep_quality_score ?? 0
  const qualityLabel = scorePercent >= 80 ? 'Good' : scorePercent >= 60 ? 'Fair' : 'Poor'
  const qualityColor = scorePercent >= 80 ? 'text-teal-600' : scorePercent >= 60 ? 'text-amber-600' : 'text-rose-600'

  const wakeDate  = s ? new Date(s.end_time)   : null
  const bedDate   = s ? new Date(s.start_time) : null
  const midDate   = bedDate && wakeDate ? new Date((bedDate.getTime() + wakeDate.getTime()) / 2) : null
  const dateLabel = bedDate ? bedDate.toLocaleDateString('en-SG', { month: 'short', day: 'numeric' }) : '--'
  ...
  const vitalsMetrics = [
    { label: 'Avg Heart Rate',   value: s ? `${s.avg_heart_rate} bpm`             : '--', tone: '' },
    { label: 'Avg Respiration',  value: s ? `${s.avg_respiration_rate} /min`       : '--', tone: '' },
    { label: 'Apnea Events',     value: s ? `${s.total_apnea_events}`              : '--', tone: s ? (s.total_apnea_events === 0 ? 'text-emerald-700' : 'text-rose-600') : '' },
    { label: 'Turns in Bed',     value: s ? `${s.total_turns}`                     : '--', tone: '' },
    { label: 'Sleep Onset',      value: s ? `${s.sleep_onset_latency_minutes} min` : '--', tone: '' },
    { label: 'WASO',             value: s ? `${s.waso_minutes} min`                : '--', tone: s ? (s.waso_minutes > 60 ? 'text-amber-600' : '') : '' },
    { label: 'Times Out of Bed', value: s ? `${s.times_out_of_bed}`                : '--', tone: '' },
    { label: 'Efficiency',       value: s ? `${s.sleep_efficiency.toFixed(1)}%`    : '--', tone: s ? (s.sleep_efficiency >= 85 ? 'text-emerald-700' : s.sleep_efficiency >= 70 ? '' : 'text-rose-600') : '' },
  ]
      ...
      <div className="space-y-3 px-4">
        ...
        {anomalies.length > 0 && (
          <div className="space-y-1.5">
            {anomalies.map((a, i) => (
              <div key={i} className="flex items-center gap-2 bg-amber-50 border border-amber-100 rounded-[14px] px-3 py-2">
          ...
          <div className="relative h-32 w-32">
            {loading ? (
              ...
              <svg
                ...
                viewBox="0 0 128 128"
              >
                <circle cx="64" cy="64" r={RING_R} fill="none" stroke="#e2e8f0" strokeWidth="12" />
                <circle
            ...
            <div className="absolute inset-0 grid place-items-center">
              <div className="grid h-24 w-24 place-items-center rounded-full bg-slate-50 text-center">
                <div>
                  <p className={`text-4xl leading-none font-semibold ${loading ? 'text-slate-300' : 'text-slate-900'}`}>
                    {loading ? '--' : scorePercent}
                  </p>
                  {!loading && (
                    <p className={`mt-0.5 text-[10px] font-bold tracking-[1px] uppercase ${qualityColor}`}>{qualityLabel}</p>
                  )}
                  <p className="text-[9px] text-slate-400 mt-0.5">Sleep Score</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {[
            { color: 'bg-cyan-600',  value: loading ? '--' : `${deepPct}%`,  label: 'Deep',  textColor: 'text-cyan-700'  },
            { color: 'bg-sky-300',   value: loading ? '--' : `${lightPct}%`, label: 'Light', textColor: 'text-sky-700'   },
            { color: 'bg-amber-200', value: loading ? '--' : `${awakePct}%`, label: 'Awake', textColor: 'text-amber-800' },
          ].map(({ color, value, label, textColor }) => (
            <Card key={label} className="rounded-2xl py-3 shadow-sm">
              <CardContent className="px-2 text-center">
                <div className={`mx-auto mb-1 h-3 w-3 rounded-sm ${loading ? 'bg-slate-200' : color}`} />
                <p className={`text-sm font-bold ${loading ? 'text-slate-300' : textColor}`}>{value}</p>
        ...
        <Card className="rounded-3xl py-4">
          <CardContent className="px-4">
            <p className="mb-3 text-[15px] font-bold text-slate-900">Sleep Architecture</p>
            ...
            {loading ? (
              ...
              <>
                  ...
                  {hasIntervals
                    ? intervals.map((iv, idx) => (
                        <div
                          ...
                          style={{ width: `${(iv.duration_minutes / s!.time_in_bed_minutes) * 100}%` }}
                        />
                      ))
                  ...
                  <div className="flex mt-1.5">
                    {intervals.map((iv, idx) => (
                      <div
                        ...
                        style={{ width: `${(iv.duration_minutes / s!.time_in_bed_minutes) * 100}%` }}
                      >
                        {iv.apnea_events > 0 && <div className="w-1.5 h-1.5 rounded-full bg-rose-500" />}
                        {iv.abnormal_struggle && <div className="w-1.5 h-1.5 rounded-full bg-amber-500" />}
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div className="mt-2 flex justify-between">
                  <p className="text-[10px] text-slate-400">{bedDate && hasIntervals ? fmtHour(bedDate) : '10 PM'}</p>
                  <p className="text-[10px] text-slate-400">{midDate && hasIntervals ? fmtHour(midDate) : '2 AM'}</p>
                  <p className="text-[10px] text-slate-400">{wakeDate && hasIntervals ? fmtHour(wakeDate) : '6 AM'}</p>
                </div>
              </>
            )}
        ...
        <Card className="rounded-3xl py-4">
          <CardContent className="px-4">
            <p className="mb-3 text-[15px] font-bold text-slate-900">Vitals During Sleep</p>
            <div className="grid grid-cols-2 gap-2">
              {vitalsMetrics.map(({ label, value, tone }) => (
                <div key={label} className="relative rounded-xl border border-slate-100 bg-slate-50 px-3 py-2">
                  <div className="flex items-center justify-between mb-0.5">
                    <p className="text-[10px] font-bold tracking-wide text-slate-400 uppercase leading-none">{label}</p>
                    <button
                  ...
                  <p className={`text-sm font-bold ${loading ? 'text-slate-300' : `text-slate-800 ${tone}`}`}>{value}</p>
                </div>
              ))}
            </div>
            ...
            {activeTooltip && (
              ...
              >
                <p className="text-[12px] font-bold text-slate-700 mb-0.5">{activeTooltip}</p>
                <p className="text-[12px] text-slate-500 leading-relaxed">{VITALS_INFO[activeTooltip]}</p>
                <p className="text-[10px] text-slate-400 mt-1.5">Tap to dismiss</p>
              </button>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="rounded-3xl py-4">
            <CardContent className="px-4">
                    ...
                    <svg viewBox={`0 0 ${TOTAL_W} ${TOTAL_H}`} className="w-full h-auto overflow-visible" onClick={() => setSelectedWeekDay(null)}>
                        ...
                        {!weeklyLoading && weekly.map((day, idx) => {
                          ...
                          return (
                            <clipPath key={`clip-${idx}`} id={`clip-${idx}`}>
                              <rect x={x} y={clipTop} width={BAR_W} height={clipBot - clipTop} rx="3" ry="3" />
                            </clipPath>
                          )
                        })}
                      </defs>
                      ...
                      {yLabels.map(({ t, label }) => (
                        <g key={t}>
                          ...
                          <text x={LEFT - 4} y={toY(t) + 4} textAnchor="end" fontSize="8.5" fontWeight="600" fill="#94a3b8">{label}</text>
                        </g>
                      ))}
              ...
              <div className="mt-2 flex items-center gap-4 flex-wrap">
                {[['bg-cyan-600', 'Deep'], ['bg-sky-300', 'Light'], ['bg-amber-200', 'Awake']].map(([color, label]) => (
                  <div key={label} className="flex items-center gap-1">
                    <div className={`h-2 w-3 rounded-sm ${color}`} />
                    <span className="text-[10px] text-slate-500">{label}</span>
                  </div>
                ))}
                {(weeklyIntervals.some(ivs => ivs.some(iv => iv.apnea_events > 0)) || weekly.some(d => d.hasApnea)) && (
                  <div className="flex items-center gap-1">
