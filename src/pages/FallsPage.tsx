    ...
    <div className="pb-5">
      ...
      <div className="space-y-2.5 px-4">
        ...
        <div className={cn(
          ...
          isFallen
            ? 'bg-rose-50'
            : 'bg-emerald-50 border border-[#a7f3d0]',
        )}>
          <div className="flex items-center gap-2.5 mb-3">
            ...
            <div>
              <p className={cn('text-[16px] font-bold leading-snug', isFallen ? 'text-[#881337]' : 'text-[#064e3b]')}>
                {isFallen ? 'Ashley has fallen' : 'No fall detected today'}
              </p>
              <p className={cn('text-[13px] mt-0.5', isFallen ? 'text-rose-500' : 'text-emerald-600')}>
                {isFallen ? `${roomLabel} · Still on floor` : `${roomLabel} · Monitoring active`}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              ['Duration', isFallen ? '2 min 14 sec' : 'Active', isFallen ? 'crit' : 'ok'],
              ['Movement', motion === 'still' ? 'Still' : motion === 'active' ? 'Active' : 'None', motion === 'still' ? 'warn' : 'ok'],
              ['Heart Rate', `${heartRate} bpm`, 'ok'],
            ].map(([label, value, tone]) => (
              <div key={label} className="bg-white border border-slate-100 rounded-[14px] p-3">
                <p className="text-[10px] font-bold tracking-[.07em] uppercase text-slate-400">{label}</p>
                <p className={cn(
                  'text-[14px] font-bold mt-0.5',
                  tone === 'crit' ? 'text-rose-500' : tone === 'warn' ? 'text-amber-500' : 'text-emerald-600',
                )}>
                  {value}
                </p>
              </div>
          ...
          {isFallen && (
            <div className="flex gap-2 mt-3">
              ...
              <button className="flex-1 py-2.5 rounded-[10px] text-[#881337] text-[13px] font-bold cursor-pointer" style={{ background: 'rgba(0,0,0,0.08)' }}>
                False Alarm
              </button>
            </div>
          )}
        ...
        <div className="bg-white rounded-[24px] border border-slate-100 shadow-sm p-4">
          <div className="flex justify-between items-center mb-3">
            <div>
              <p className="text-[12px] font-bold text-slate-700">Fall Risk Level</p>
              <p className="text-[11px] text-slate-400 mt-0.5">Based on current sensor data</p>
            </div>
            <span className={cn('text-[13px] font-bold', isFallen ? 'text-rose-500' : 'text-emerald-600')}>
              {isFallen ? 'High' : 'Low'}
            </span>
          </div>
          ...
          <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
            <div
              className={cn('h-full rounded-full transition-all duration-700', isFallen ? 'bg-rose-500' : 'bg-emerald-500')}
